package com.game.modules.view.task
{
   ...
   public class CellPhoneToUrl extends Sprite
   {
      ...
      public function CellPhoneToUrl()
      {
         super();
      }
      
      public function initView(url:String) : void
      {
         if(url == null || url == "")
         {
            return;
         ...
         this.loader = new Hloader(URLUtil.getSvnVer(url));
      ...
      private function showDia() : void
      {
         ...
         this.mc.dia.htmlText = this.nameFilter(this.currentDia);
         if(this.data.desc.length == 0)
         {
            this.mc.next.removeEventListener(MouseEvent.CLICK,this.onClickNext);
            this.mc.close.addEventListener(MouseEvent.CLICK,this.onClickClose);
