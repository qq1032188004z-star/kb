...
{
   import flash.utils.*;
   import org.zip.util.ChecksumUtil;
   import org.zip.wirelust.as3zlib.JZlib;
   import org.zip.wirelust.as3zlib.System;
   import org.zip.wirelust.as3zlib.ZStream;
   ...
   public class FZipFile
   {
      ...
      public function serialize(stream:IDataOutput, includeAdler32:Boolean, centralDir:Boolean = false, centralDirOffset:uint = 0) : uint
      {
         ...
         var fileSize:uint = 0;
         if(!centralDir && this._sizeCompressed > 0)
         {
            if(HAS_INFLATE)
            {
      ...
      private function parseFileContent(stream:IDataInput) : Boolean
      {
         ...
         {
            if(stream.bytesAvailable < this._sizeCompressed)
            {
               return false;
            }
      ...
      {
         var uncompr:ByteArray = null;
         var d_stream:ZStream = null;
         var err:int = 0;
         var i:int = 0;
         if(this.isCompressed && this._content.length > 0)
         {
            ...
            {
               uncompr = new ByteArray();
               d_stream = new ZStream();
               d_stream.next_in = this._content;
               d_stream.next_in_index = 0;
               d_stream.next_out = uncompr;
               d_stream.next_out_index = 0;
               err = d_stream.inflateInitWithNoWrap(true);
               i = 0;
               while(d_stream.total_in <= this._content.length && i <= this._content.length * 4)
               {
